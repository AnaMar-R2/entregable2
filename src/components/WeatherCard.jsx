  ...
  return (
    <article className='weather'>
      ...
      <footer className='weather__footer'>
        <h2 className='weather__temp'>{isCelsuis ? `${temp?.celsius} °C` : `${temp.farenheit} °F`}</h2>
        <button className='weather__btn' onClick={handleChangeTemp}>Change to {isCelsuis ? '°F' : '°C'}</button>
      </footer>
    </article>
  )
}
